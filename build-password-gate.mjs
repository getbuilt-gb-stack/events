...
const eventConfig = {
  eventName: process.env.REPORT_EVENT_NAME || "",
  eventDate: process.env.REPORT_EVENT_START_DATE || "",
  eventEndDate: process.env.REPORT_EVENT_END_DATE || "",
  campaignId: process.env.REPORT_CAMPAIGN_ID || "",
  webhookUrl: process.env.REPORT_EVENT_WEBHOOK_URL || "",
  feedbackSpreadsheetUrl: process.env.REPORT_FEEDBACK_SPREADSHEET_URL || "",
  feedbackSheetId: process.env.REPORT_FEEDBACK_SHEET_ID || "",
};
const missingEventConfig = Object.entries(eventConfig)
  .filter(([, value]) => !value)
  .map(([name]) => name);
if (missingEventConfig.length) {
  throw new Error(`Missing event build configuration: ${missingEventConfig.join(", ")}`);
}
...
const originalInteractionMetaGrid = '<div class="interaction-grid"><label>Interaction type<select name="interactionType"><option>Conversation</option><option>Meeting</option><option>Session</option><option>Product Expo</option><option>Reception</option><option>Virtual meeting</option><option>Other</option></select></label><label>Date and time<input name="occurredAt" type="datetime-local"></label>';
const reorderedEventInteractionMarkup = newEventInteractionMarkup
  ...
  .replace(originalInteractionMetaGrid, '')
